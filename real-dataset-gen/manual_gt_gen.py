...
class PoseInitSidebar(BoxLayout):
    def __init__(self, left_img, right_img, pose_init):
        ...
        self.show_overlap_btn = Button(text="Show overlap", size_hint=(1.0,None), height=30)
...
class PoseInit():
    ...
    @staticmethod
    def blend_imgs(real, rend, alpha):
        mask = np.where(rend>0, 1.0, 0.0)
        real_masked = real*mask
        real_masked = np.uint8(real_masked*255.0)
        rend = np.uint8(rend*255.0)
        real_masked = PIL.Image.fromarray(real_masked)
        rend = PIL.Image.fromarray(rend)
        blend = np.asarray(PIL.Image.blend(real_masked,rend,alpha))
        rend = np.asarray(rend)/255.0
        blend = blend/255.0
        overlapped = np.where(mask>0, blend, real)
        return overlapped

    def overlap_imgs(self):
        real = cv2.cvtColor(self.get_real_image(), cv2.COLOR_BGR2RGB)/255.0
        rend = self.get_rendered_image()
        rend = self.create_silhouette(rend)
        mask = np.uint8(np.where(rend>0, 255, 0))[:,:,0]

        rend_out = cv2.cvtColor(np.uint8(rend*255), cv2.COLOR_RGB2BGR)
        rend_out = np.dstack((rend_out, np.zeros((rend.shape[0], rend.shape[1]), dtype=np.uint8)))
        rend_out[:,:,3] = mask
        cv2.imwrite("test.png", rend_out)
        print(rend_out.shape)

        blend = self.blend_imgs(real, rend, 0.75)
        plt.imshow(blend)
        plt.show()
